    ...
    <nav className="bg-[#002A5C] font-source z-50 fixed w-full">
      <div className="w-full flex items-center h-20 md:h-28 lg:h-36 justify-between">
        <Link className="pt-3 px-4 md:px-12 lg:px-20 flex items-center" href="/">
          <Image src="/EWB-White.png" alt="EWB logo" width={200} height={58} className="w-20 md:w-32 lg:w-40 xl:w-48 h-auto" priority />
            ...
            >
              {item.dropdown ? (
                <div className="relative">
                  <span className={`whitespace-nowrap transition-colors duration-300 cursor-pointer ${pathname.startsWith(item.href) ? 'text-orange-400' : 'text-white hover:text-orange-400'
                    } text-[16px]`}>
                    {item.label}
                  </span>
                    ...
                    <div className="absolute top-full left-0 bg-[#002A5C] border border-gray-600 py-2 min-w-[200px] z-50">
                      {item.dropdown.map((dropdownItem) => (
                        <Link
                          ...
                          className={`block px-4 py-2 text-sm transition-colors duration-300 ${pathname === dropdownItem.href
                              ? 'text-orange-400'
                ...
                <Link href={item.href} className={`whitespace-nowrap transition-colors duration-300 ${pathname === item.href ? 'text-orange-400' : 'text-white hover:text-orange-400'
                  } text-[16px]`}>
                  {item.label}
                </Link>
              )}
          ...
          <ul className="flex flex-col gap-4">
            {navItems.map((item) => (
              <li key={item.href}>
                {item.dropdown ? (
                  <div>
                    <span className={`block transition-colors duration-300 ${pathname.startsWith(item.href) ? "text-orange-400" : "text-white"
                    ...
                    <div className="ml-2 space-y-1">
                      {item.dropdown.map((dropdownItem) => (
                        <Link
                          key={dropdownItem.href}
                          href={dropdownItem.href}
                          className={`block px-3 py-2 transition-colors duration-300 border-l-3 ${pathname === dropdownItem.href
                  ...
                  <Link
                    href={item.href}
                    className={`block transition-colors duration-300 ${pathname === item.href
                        ? "text-orange-400"
                        : "text-white hover:text-orange-400"
                      } text-[16px]`}
                    onClick={() => setIsOpen(false)}
