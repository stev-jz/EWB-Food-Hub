...
interface FilterDropdownProps {
    ...
    displayTransform?: (val: any) => string
    isOpen: boolean
    filters: Filters
...
export default function FilterDropdown({ 
    ...
    label, 
    options, 
    displayTransform = (val: any) => val,
    isOpen,
    filters,
    onToggle,
    onToggleDropdown
}: FilterDropdownProps) {
    const count = (filters[category] as any[]).length
    const isPriceRange = category === 'priceRange'
    const activeCount = isPriceRange && count === 4 ? 0 : count
    
    return (
        <div className="relative">
            <button
                onClick={() => onToggleDropdown(category)}
                className={`flex items-center gap-1.5 px-2 py-1 text-xs border border-black rounded transition-colors cursor-pointer ${
                    activeCount > 0 
            ...
            {isOpen && (
                <div className="absolute top-full left-0 mt-1 w-40 bg-white border border-black rounded shadow-lg z-10 max-h-48 overflow-y-auto">
                    <div className="p-1.5 space-y-0.5">
                        {options.map(option => {
                            const isSelected = (filters[category] as any[]).includes(option)
                            return (
